...
export const ModalNewService = () => {
	...
	const { serviceCategory, serviceInfo, serviceName, cityName, postalCode, street } = formValues;
	
	
	const state = useSelector(state => state.auth)
	const idModal = 'NewService' + state.user.uid;
	...
	const handleSubmit = (e) => {
		e.preventDefault();
		if (isFormValid()) {
			dispatch(addNewService(serviceName, serviceInfo,serviceCategory,street,postalCode,cityName));
		}
	}
	return (
			...
			<div className="modal-dialog modal-dialog-centered">
				<div className="modal-content">
					<div className="modal-header">
						<h5 className="modal-title" id={idModal}>Registro de servicio.</h5>
