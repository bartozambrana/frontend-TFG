...
export const ModalEditWork = ({uidWork}) => {
	...
	let initialState = useRef(work.current.photos.map((photo)=>{return {photo,selected:false}}));
	...
	useEffect(()=> {
		...
		initialState.current = work.current.photos.map((photo)=>{return {photo,selected:false}});
    ...
    const isFormValid = () => {
		...
		if(
			...
			Swal.fire('Error', 'No puede actualizar un trabajo sin modificaciones', 'error');
			return false;
		} 
		return true;
	}
	...
	const handleSubmit = (e)=>{
		...
		photosDeleted.forEach(element => {
			if(element.selected){
				if(deletedFiles === '')
					deletedFiles = element.photo;
				else
					deletedFiles = deletedFiles + ';' + element.photo
			}
			...
			<div className="modal-dialog modal-dialog-centered">
				<div className="modal-content">
					<div className="modal-header">
						<h5 className="modal-title" id={idModal}>Modificación de trabajo.</h5>
							...
							<div className="container">
								{
									...
									photosDeleted.map((photoItem,index)=>{
										return(
											<div className='mt-3' style={{cursor: 'pointer'}} key={index} onClick={()=>togglePhotoSelected(index)}>	
											{(photoItem.selected) && (<i className="fa fa-check" style={styleCheck}aria-hidden="true"></i>)}
												<img src={photoItem.photo} className={(photoItem.selected) ? 'w-100 border border-success' : 'w-100'} alt={'picture ' + index} />
